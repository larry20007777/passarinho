...
function movePipes() {
    pipes.forEach(pipe => {
        ...
        pipe.topPipe.style.left = pipeLeft - 3 + 'px';
        pipe.bottomPipe.style.left = pipeLeft - 3 + 'px';

        if (
            pipeLeft < 60 && 
            pipeLeft + pipeWidth > 0 &&
            (birdY < parseInt(pipe.topPipe.style.height) || birdY + 60 > gameContainer.clientHeight - parseInt(pipe.bottomPipe.style.height))
        ) {
            gameOver(); 
        }
